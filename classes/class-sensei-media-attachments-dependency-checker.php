 ...
 * @since 2.0.0
 */
class Sensei_Media_Attachments_Dependency_Checker {
	...
	public static function add_php_notice() {
		...
		$message = sprintf( __( '<strong>Sensei LMS Media Attachments</strong> requires a minimum PHP version of %1$s, but you are running %2$s.', 'sensei-media-attachments' ), self::MINIMUM_PHP_VERSION, phpversion() );
	...
	public static function add_sensei_notice() {
		...
		$message = sprintf( __( '<strong>Sensei LMS Media Attachments</strong> requires that the plugin <strong>Sensei</strong> (minimum version: <strong>%1$s</strong>) is installed and activated.', 'sensei-media-attachments' ), self::MINIMUM_SENSEI_VERSION );
		echo '<div class="error"><p>';
		echo wp_kses( $message, array( 'strong' => array() ) );
		echo '</p></div>';
	}
